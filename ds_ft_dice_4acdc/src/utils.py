...
def load_data_pairs(pair_list, resize_r, rename_map):
    ...
    for k in range(0, len(pair_list), 2):
        ...
        resize_dim = np.array([img_data.shape[0], img_data.shape[1], 32]).astype('int')
...
def get_batch_patches(img_clec, label_clec, patch_dim, batch_size, chn=1, rot_flag=True):
    """generate a batch of paired patches for training"""
    batch_img = np.zeros([batch_size, patch_dim[0], patch_dim[1], patch_dim[2], chn]).astype('float32')
    batch_label = np.zeros([batch_size, patch_dim[0], patch_dim[1], patch_dim[2]]).astype('int32')
    ...
    for k in range(batch_size):
        ...
        l_rand = np.arange(l - patch_dim[0])
        w_rand = np.arange(w - patch_dim[1])
        h_rand = np.arange(h - patch_dim[2])
        np.random.shuffle(l_rand)
        np.random.shuffle(w_rand)
        np.random.shuffle(h_rand)
        pos = np.array([l_rand[0], w_rand[0], h_rand[0]])
        # crop
        img_temp = copy.deepcopy(rand_img[pos[0]:pos[0]+patch_dim[0], pos[1]:pos[1]+patch_dim[1], pos[2]:pos[2]+patch_dim[2]])
        # normalization
        ...
        label_temp = copy.deepcopy(rand_label[pos[0]:pos[0]+patch_dim[0], pos[1]:pos[1]+patch_dim[1], pos[2]:pos[2]+patch_dim[2]])

        # possible augmentation
        # rotation
        if rot_flag and np.random.random() > 0.70:
    ...
    for R in range(0, fold[0]):
        ...
        for C in range(0, fold[1]):
            ...
            for H in range(0, fold[2]):
                ...
                cube_batch = np.zeros([batch_size, cube_size[0], cube_size[1], cube_size[2], n_chn]).astype('float32')
...
def compose_label_cube2vol(cube_list, vol_dim, cube_size, ita, class_n):
    ...
    idx_classes_mat = (np.zeros([cube_size[0], cube_size[1], cube_size[2], class_n])).astype('int32')
...
def remove_complex_cc(vol_data, rej_ratio, rename_map):
    ...
    # firstly remove large but simple outliers
    binary_vol = (vol_data > 0)*1
    # dilate to ensure the wanted structure to be connected together
    dilate_vol = ndimage.binary_dilation(binary_vol, iterations=3).astype(binary_vol.dtype)

    labeled_cc, num_cc = measurements.label(dilate_vol)
    class_vol_size = np.sum(dilate_vol)
    # retrieve all connected components in the class (ignoring background)
    for cc in range(1, num_cc+1):
        single_cc = ((labeled_cc==cc)*1)
        single_vol = np.sum(single_cc)
        # === 1 === remove if too small
        if single_vol / (class_vol_size*1.0) > rej_ratio:
            rem_vol[labeled_cc==cc] = 1
        # === 2 === remove if it contains too few classes
        class_retain_vol = vol_data*single_cc
        class_retain = np.unique(class_retain_vol)
        if (len(class_retain)-1) >= (len(rename_map)-1):
            rem_vol[labeled_cc==cc] = 1
    #
